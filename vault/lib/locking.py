...
#    https://www.assembla.com/code/fpdb-gimick/git/nodes/pyfpdb/interlocks.py
#    Apparently Authored by fpdb-mme
...
Differnces compared to thread locks:
...
3. acquire() can take a 3rd parameter retry_time, which, if wait is True, tells the locking mechanism how long to sleep between retrying the lock.  Has no effect for unix/InterProcessLockFcntl.
"""
...
class InterProcessLockBase:
    ...
    def acquire_impl(self, wait): 
        raise Exception("Calling abstract function")
